...
class Project_markdown_writer implements IProject_export_writer
{
	...
	public function generate($project_id, $output_file, $options = array())
	{
		...
		if (isset($options['provenance']) && $options['provenance'] instanceof Metadata_provenance) {
			$provenance = $this->construct_provenance_message($options['provenance']);
	...
	private function construct_provenance_message(Metadata_provenance $provenance): string
	{
		$markdown_divider = "\n\n***\n\n";
		$human_readable_datetime_utc = $provenance->generated_on_utc->format("F j, Y, g:i a");
		$human_readable_datetime_new_zealand = $provenance->generated_on_new_zealand_time->format("F j, Y, g:i a");

		$markdown_provenance_message = "### " . Metadata_provenance::TITLE . $markdown_divider . Metadata_provenance::GENERATED_FROM_TEXT . " on " . $human_readable_datetime_utc . " (UTC) / " . $human_readable_datetime_new_zealand . " (New Zealand time - Pacific/Auckland timezone).\n\n"
			. "Generated by: " . $provenance->user_name . $markdown_divider;
			
		return $markdown_provenance_message;
	}
}
